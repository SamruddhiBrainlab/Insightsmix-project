...
const ModelSummary = ({ selectedProject }) => {
  ...
  useEffect(() => {
    ...
    fetch(url)
      .then((response) => {
        if (!response.ok) {
          throw new Error(`Failed to fetch the MMM model summary report for project ${selectedProject}`);
        }
      ...
      .catch((error) => {
        console.error("Error fetching MMM model summary report:", error);
      ...
      >
        <Box sx={{ textAlign: 'center', p: 4 }}>
          ...
          <Typography sx={{ mt: 2, color: 'text.secondary' }}>
            Loading MMM model summary report...
          </Typography>
        </Box>
      </Paper>
    );
